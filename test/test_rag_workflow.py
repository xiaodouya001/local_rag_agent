...
def test_rag_workflow():
    ...
    questions = [
        "什么是RAG",
        "RAG 有哪些主要特点？",
        "如果要实现一个 RAG 系统，需要哪些技术组件？",
        "RAG 技术适合解决哪些类型的问题？"
    ]
    
    for idx, question in enumerate(questions, 1):
        ui.print_header(f"问题 {idx}/{len(questions)}: {question}", icon='question', color='magenta')
        ...
        ui.print_box(
            source_documents[0].page_content[:200] + "..." if source_documents else "无",
            title="原始文档（检索到的内容）",
            color='cyan'
        )
        ui.print_box(
