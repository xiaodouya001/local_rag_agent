...
class ConsoleUI:
    ...
    @staticmethod
    def print_header(text: str, icon: Optional[str] = None, color: str = 'cyan'):
        """打印标题"""
    ...
    @staticmethod
    def print_section(text: str, icon: Optional[str] = None, color: str = 'blue'):
        """打印章节标题"""
        icon_str = f"{ConsoleUI._get_icon(icon)} " if icon else ""
    ...
    @staticmethod
    def print_box(content: str, title: Optional[str] = None, color: str = 'cyan'):
        """打印带边框的文本"""
        lines = content.split('\n')
            ...
            if len(line) > max_width:
                ...
                for word in words:
                    if len(current_line + word) <= max_width:
                        ...
                        if current_line:
                            print(f"{color_code}│ {current_line.strip():<{max_width}} │{Style.RESET_ALL}")
                        current_line = word + " "
                if current_line:
                    print(f"{color_code}│ {current_line.strip():<{max_width}} │{Style.RESET_ALL}")
            else:
    ...
    @staticmethod
    def print_step(step_num: int, text: str, icon: Optional[str] = None):
        """打印步骤"""
        icon_str = f"{ConsoleUI._get_icon(icon)} " if icon else ""
    ...
    def print_progress(text: str):
        """打印进度信息"""
        print(f"{Fore.CYAN}{Style.DIM}⏳ {text}...{Style.RESET_ALL}", end='', flush=True)
    ...
    @staticmethod
    def print_list(items: List[str], title: Optional[str] = None, icon: str = 'dot'):
        """打印列表"""
        if title:
    ...
    def print_table(headers: List[str], rows: List[List[str]], title: Optional[str] = None):
        ...
        print(f"{Fore.CYAN}{'─' * (sum(col_widths) + 3 * (len(headers) - 1))}{Style.RESET_ALL}")
